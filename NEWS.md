...
+ This unlocks a range of new functionalities for `boottest()` with `fixest` objects - it is now possible to run `boottest()` after `feols()` models that use syntactic sugar:
