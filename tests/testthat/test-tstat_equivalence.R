...
wald_test <- function(run_this_test){
    ...
    if(run_this_test){
      ...
      boot_jl <- suppressWarnings(mboottest(floattype = "Float64", lm_fit, R = R, clustid = "group_id1", B = 999))
      ...
      boot_jl <- suppressWarnings(
        suppressWarnings(fwildclusterboot::mboottest(floattype = "Float64",
                                  object = lm_fit,
                                  R = R,
                                  clustid = "group_id1",
                                  B = 999,
      ...
      # one hypothesis
      R <- clubSandwich::constrain_zero(constraints = 2, coefs = coef(lm_fit))
      boot_jl <- suppressWarnings(fwildclusterboot::mboottest(floattype = "Float64",
                                                            lm_fit,
                                                            R = R,
                                                            clustid = c("group_id1", "group_id2"),
                                                            B = 999,
      ...
      R <- clubSandwich::constrain_zero(constraints = 1:2, coefs = coef(lm_fit))
      boot_jl <- suppressWarnings(
        fwildclusterboot::mboottest(floattype = "Float64",
                                  object = lm_fit,
                                  R = R,
                                  clustid = "group_id1",
                                  B = 999,
