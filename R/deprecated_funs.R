invert_p_val.algo_multclust <- function(object, point_estimate, se_guess, clustid, fixed_effect, v,X, Y, N, k, param, R0, B, beta0, alpha, W, n_fe, N_G, ...){
  ...
  p_val_null <- function(beta0, Q, P, R0, X, XinvXXr, XinvXXrX, clustid, 
    ...
    if(is.null(W)){
      ...
      for(x in names(clustid)){
        ...
        K <- S_diag_XinvXXRu_S - tcrossprod(SXinvXXrX %*% invXX, SuX) # c* x x
      ...
      # crosstab2: clustid["clustid"]; crosstab
      ...
      for(x in names(clustid)){
        ...
        prod <- t(tcrossprod(S_Wu_F, S_XinvXXR_F)) # c* x c*
        #diag_SXinvXXRu <- Matrix::Diagonal(N_G["clustid"], SXinvXXRu)
        S_diag_XinvXXRu_S <- S_diag_XinvXXRu_S - prod      
        # stop projecting out fixed effect
        
        SXinvXXrX <-  collapse::fsum(XinvXXrX, clustid[x])
        K <- S_diag_XinvXXRu_S - tcrossprod(SXinvXXrX %*% invXX, SuX)
        tKK[[x]] <- small_sample_correction[x] * Matrix::t(K) %*% K # here: add small sample df correction
      }
    }
  ...
  p_val_null_x <- function(beta0){
    p_val_null(beta0, Q = Q, P = P, R0 = R0, X = X, XinvXXr = XinvXXr, XinvXXrX = XinvXXrX, clustid = clustid, v = v, B = B, small_sample_correction = small_sample_correction) - alpha
  }
