...
#' data(voters)
#' plm_fit <- plm(proposition_vote ~ treatment + ideology1 + log_income + Q1_immigration,
#'   data = voters
#' )
...
boottest.plm <- function(object,
  ...
  if(object$call$model != "pooling"){
    stop("Currently, only models of type `plm` with `model = 'pooling'` are supported.")
  }
