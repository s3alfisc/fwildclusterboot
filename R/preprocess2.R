...
get_cluster2 <- function(object, clustid_char, bootcluster, N, call_env) {
  ...
  cluster_bootcluster_fml <- update(clustid_fml, paste("~ . +", paste(bootcluster_char, collapse = " + ")))
  ...
  N_G <- vapply(cluster, function(x) length(unique(x)), numeric(1))

  # now do all the other bootcluster things
  c1 <- bootcluster_char[which(!(bootcluster_char %in% clustid_char))]
  # both bootstrapping and error cluster: all variables in clustid_char that are also in bootcluster
