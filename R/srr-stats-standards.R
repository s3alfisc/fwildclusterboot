...
#' @srrstats {G1.6} *Software should include code necessary to compare performance claims with alternative implementations in other R packages.* Is a link to this blog post sufficient? https://s3alfisc.github.io/blog/post/1000x-faster-wild-cluster-bootstrap-inference-in-r-with-fwildclusterboot/
...
#' @srrstats {G2.11} *Software should ensure that `data.frame`-like tabular objects which have columns which do not themselves have standard class attributes (typically, `vector`) are appropriately processed, and do not error without reason. This behaviour should be tested. Again, columns created by the [`units` package](https://github.com/r-quantities/units/) provide a good test case.* This I don't understand.
#' @srrstats {G2.12} *Software should ensure that `data.frame`-like tabular objects which have list columns should ensure that those columns are appropriately pre-processed either through being removed, converted to equivalent vector columns where appropriate, or some other appropriate treatment such as an informative error. This behaviour should be tested.* Does not happen.
#' @srrstats {G2.13} *As `boottest()` is a post-estimation command, missing data checks are only required for variables not included in the original regression model. The respective regression functions ensure that no NA values are inlcuded when fittint the model.Missing data may occur only if the user chooses a clustering variable that is not included in the original model (the linear regression) & which includes NA values. In this case, `boottest()` throws an error.*
#' @srrstats {G2.14} *see G2.13*
#' @srrstats {G2.14a} *see G2.13*
#' @srrstats {G2.14b} *see G2.13*
#' @srrstats {G2.14c} *see G2.13*
#' @srrstats {G2.15} *Functions should never assume non-missingness, and should never pass data with potential missing values to any base routines with default `na.rm = FALSE`-type parameters* Done
...
#' @srrstats {G4.0} *Statistical Software which enables outputs to be written to local files should parse parameters specifying file names to ensure appropriate file suffices are automatically generated where not provided.* No output can be written to local files.
#' @srrstats {G5.0} *Where applicable or practicable, tests should use standard data sets with known properties (for example, the [NIST Standard Reference Datasets](https://www.itl.nist.gov/div898/strd/), or data sets provided by other widely-used R packages).* All tests based on random data sets.
#' @srrstats {G5.1} *Data sets created within, and used to test, a package should be exported (or otherwise made generally available) so that users can confirm tests and run examples.* Data sets used internally can be recreated via a non-exported `fwildclusterboot:::create_data()` function.
#' @srrstats {G5.2} *Appropriate error and warning behaviour of all functions should be explicitly demonstrated through tests. In particular,* Tested in test-warning-error test file.
#' @srrstats {G5.2a} *Every message produced within R code by `stop()`, `warning()`, `message()`, or equivalent should be unique* Done.
#' @srrstats {G5.2b} *Explicit tests should demonstrate conditions which trigger every one of those messages, and should compare the result with expected values.* Done. See test-warning-error test file.
#' @srrstats {G5.4} **Correctness tests** *to test that statistical algorithms produce expected results to some fixed test data sets (potentially through comparisons using binding frameworks such as [RStata](https://github.com/lbraglia/RStata)).* Several correctness tests are implemented. First, it is tested if the non-bootstrapped t-statistics
#' produced via boottest() *exactly* match those computed by the fixest package (see test_tstat_equivalence). Second, `fwildclusterboot` is heavily tested against `WildBootTests.jl` - see "test-r-vs-julia".
#' @srrstats {G5.4b} *For new implementations of existing methods, correctness tests should include tests against previous implementations. Such testing may explicitly call those implementations in testing, preferably from fixed-versions of other software, or use stored outputs from those where that is not possible.* Extensive tests against WildBootTests.jl
#' @srrstats {G5.5} *Correctness tests should be run with a fixed random seed* Done.
#' @srrstats {G5.6} **Parameter recovery tests** *to test that the implementation produce expected results given data with known properties. For instance, a linear regression algorithm should return expected coefficient values for a simulated data set generated from a linear model.* Done. Non-bootstrapped t-stats are tested against t-stats and F-stats computed by the fixest package (see test_tstat_equivalence.R). Also, tests if bootstrapped p-values are deterministic under "full enumeration" (test-seed.R).
#' @srrstats {G5.6a} *Parameter recovery tests should generally be expected to succeed within a defined tolerance rather than recovering exact values.* t-stat equivalence is tested "exactly", r vs Julia is tested with tolerance.
#' @srrstats {G5.6b} *Parameter recovery tests should be run with multiple random seeds when either data simulation or the algorithm contains a random component. (When long-running, such tests may be part of an extended, rather than regular, test suite; see G4.10-4.12, below).* Done.
#' @srrstats {G5.7} **Algorithm performance tests** *to test that implementation performs as expected as properties of data change. For instance, a test may show that parameters approach correct estimates within tolerance as data size increases, or that convergence times decrease for higher convergence thresholds.* See test-convergence.R, which tests if the bootstrap converges in probability for B -> infinity.
#' @srrstats {G5.8} **Edge condition tests** *to test that these conditions produce expected behaviour such as clear warnings or errors when confronted with data with extreme properties including but not limited to:* Tests for non-randomness under full enumeration:
#' @srrstats {G5.8c} *Data with all-`NA` fields or columns or all identical fields or columns* If all cluster vars are NA, this leads to an error.
#' @srrstats {G5.9b} *Running under different random seeds or initial conditions does not meaningfully change results* See the convergence tests.
...
#' @noRd
#' @srrstatsNA {G1.5} *Software should include all code necessary to reproduce results which form the basis of performance claims made in associated publications.* There are no associated publications.
#' @srrstatsNA {G2.4b} *explicit conversion to continuous via `as.numeric()`* Does not happen.
#' @srrstatsNA {G2.4e} *explicit conversion from factor via `as...()` functions* Does not happen.
...
#' @srrstatsNA {G2.9} *Software should issue diagnostic messages for type conversion in which information is lost (such as conversion of variables from factor to character; standardisation of variable names; or removal of meta-data such as those associated with [`sf`-format](https://r-spatial.github.io/sf/) data) or added (such as insertion of variable or column names where none were provided).* Type conversion with information loss never happens.
#' @srrstatsNA {G5.3} *For functions which are expected to return objects containing no missing (`NA`) or undefined (`NaN`, `Inf`) values, the absence of any such values in return objects should be explicitly tested.* Multiple tests for output correctness, but not dedicated tests for NA values.
#' @srrstatsNA {G5.10} *Extended tests should included and run under a common framework with other tests but be switched on by flags such as as a `<MYPKG>_EXTENDED_TESTS=1` environment variable.* TO do. Currently, I only run "test-julia-vs-r" locally as codecov fails (these tests take too long). Here I would need some help to set this up properly.
#' @srrstatsNA {G5.11} *Where extended tests require large data sets or other assets, these should be provided for downloading and fetched as part of the testing workflow.* All tests rely on internally created data sets.
#' @srrstatsNA {G5.11a} *When any downloads of additional data necessary for extended tests fail, the tests themselves should not fail, rather be skipped and implicitly succeed with an appropriate diagnostic message.* All tests rely on internally created data sets.
...
#' @srrstatsNA {G5.8d} *Data outside the scope of the algorithm (for example, data with more fields (columns) than observations (rows) for some regression algorithms)* Needs to be handled by the regression package.
#' @srrstatsNA {G5.9} **Noise susceptibility tests** *Packages should test for expected stochastic behaviour, such as through the following conditions:* This might not make sense for a bootstrap package?
...
#' @srrstatsNA {RE4.7} *Where appropriate, convergence statistics* I think supplying convergence stats for the test inversion procedure would be too much.
...
#' @srrstatsNA {RE4.13} *Predictor variables, and associated "metadata" where applicable.*
