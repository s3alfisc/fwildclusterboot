...
invert_p_val2 <- function(object, B, point_estimate, se_guess, clustid, alpha, vcov_sign, impose_null, p_val_type) {
  ...
  p_val_null2_x <- function(beta0, alpha) {
    p_val_null2(beta0, A = A, B = B, CC = CC, CD = CD, DD = DD, clustid = clustid, boot_iter = boot_iter, small_sample_correction = small_sample_correction, point_estimate = point_estimate, impose_null = impose_null, p_val_type = p_val_type)$p_val - alpha
  }
