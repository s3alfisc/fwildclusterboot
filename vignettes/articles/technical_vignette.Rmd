---
title: "The (Wild) Cluster Bootstrap and Implementation Details - Preliminary Draft"
---
...
$$
  \hat{V} = (X'X)^{-1} (\sum_{g=1}^{G} X_g' \hat{u}_g \hat{u}_g' X_g) (X'X)^{-1} 
$$ 
...
$$
  [\hat\beta +/- \text{crit. value} \times \hat{SE}(\beta)].
$$
...
Details on the implementation of the wild cluster bootstrap in 'WildBootTests.jl' can be found on [github](https://github.com/droodman/WildBootTests.jl/tree/master/in-case-i-get-hit-by-a-bus).