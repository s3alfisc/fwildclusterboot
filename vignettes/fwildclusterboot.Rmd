...
This in turn has a consequence for the use of `boottest()` and `modelsummary`. `boottest()` simply calls the `glance` methods for objects of types `fixest`, `felm` and `lm`, and therefore, the number of observations reported via `msummary()` is the number of observations used in the modeling stage. 

The default behavior of `boottest()` - to delete missings with a warning - can be set off via the `na_omit` function argument. If `na_omit` is set to FALSE, `boottest()` will not allow for missing values and throw an error. 
